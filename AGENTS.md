...
- Retrieval: [`src/lib/rag/store.ts`](src/lib/rag/store.ts) — Qdrant hybrid (`QDRANT_URL`)
- Chat: MongoDB [`src/lib/memory/sessions.ts`](src/lib/memory/sessions.ts) (`MONGODB_URI`)
- Secrets: `.env.local` only
- Commands: `docker compose up -d`, `npm run dev`, `npx tsc --noEmit`, `npm run lint`, `npm run eval`
- Docs: [prd.md](prd.md), [tech-stack.md](tech-stack.md), [decision.md](decision.md), [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md), [docs/EVALUATION.md](docs/EVALUATION.md)
